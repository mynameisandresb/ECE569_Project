...
void gaussian_filter_kernel(unsigned char* d_frame,
  ...
  for (int i = halfway_point; i <= -halfway_point; --i){ 
    for (int j = halfway_point; j <= -halfway_point; --j){ 
            ...
            int h = ((float)(r + i), 0.f), (numRows-1)); 
            int w = ((float)(c + j), 0.f), (numCols-1)); 
            float current_pixel = static_cast<float>(d_frame[w + numCols * h]); 

            // now, get the associated weight in the filter
            float weight = d_gfilter[(i + halfway_point) * d_filter_width + j + halfway_point]; 
